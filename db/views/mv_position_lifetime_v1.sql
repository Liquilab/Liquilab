...
DROP MATERIALIZED VIEW IF EXISTS "mv_position_lifetime_v1" CASCADE;
...
SELECT
    ...
    MAX(
        CASE
            WHEN pe."owner" IS NOT NULL AND pe."owner" <> '' THEN pe."owner"
            WHEN pe."recipient" IS NOT NULL AND pe."recipient" <> '' THEN pe."recipient"
            ELSE NULL
        END
    ) AS last_known_owner,
