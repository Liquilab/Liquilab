-- mv_sparkdex_rewards_24h
-- Aggregates SparkDEX TokenDistributor rewards (SPX, rFLR) per pool over the last ~24h (7200 blocks).
-- USD conversion is placeholder (0) until price integration is added.

DROP MATERIALIZED VIEW IF EXISTS "mv_sparkdex_rewards_24h" CASCADE;

CREATE MATERIALIZED VIEW "mv_sparkdex_rewards_24h" AS
...
window_rewards AS (
  ...
  WHERE r.block_number >= COALESCE(lb.max_block, 0) - 7200
)
SELECT
  pool_address,
...
CREATE UNIQUE INDEX IF NOT EXISTS mv_sparkdex_rewards_24h_pool_token_idx
  ON "mv_sparkdex_rewards_24h" (pool_address, reward_token_address);
